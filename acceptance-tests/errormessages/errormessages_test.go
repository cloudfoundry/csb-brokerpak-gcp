...
var _ = Describe("Error Messages", func() {
	...
	When("the service creation fail asynchronously", func() {
		It("puts a useful error message in the service description", func() {
			...
			stdout, _ := cf.Run("service", name)
			Expect(stdout).To(MatchRegexp(`message:\s+Error: googleapi: Error 400: Unknown project id: not-real-project, invalid  on brokertemplate/definition.tf line 16, in resource "google_storage_bucket" "bucket"`))
		})
	})
})
